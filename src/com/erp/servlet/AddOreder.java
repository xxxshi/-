 ...
 * @create: 2018-12-18 18:04
 */
@WebServlet(urlPatterns = "/AddOreder")
public class AddOreder extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //if(flag==1) {
            ...
            if (isSuccess) {
                for (int i = 0; i < itemid.length; i++) {
                    String detailSql = "insert into orderdetail values(?,?,?,?,?,?,?,?,?,?,?)";
                ...
                request.getRequestDispatcher("/ShowOrderMaster").forward(request, response);
            }else{
                request.setAttribute("errorInfo","添加销售订单失败(可能销售订单号已存在或输入数据有误)");
